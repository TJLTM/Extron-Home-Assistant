...
copied and modified from the origianl APC sensor component
the name should be the same as in the multiapcupsd configuration. 

example YAML 
sensor:
  - platform: mapc_sensors
    name: APC1
    resources:
      - bcharge
      - linev
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if config['name']:
        name = config['name']
        if config['resources']:
            for sensoritem in config['resources']:
                if sensoritem in SENSOR_TYPES:
                    entities.append(APCUPSdSensor(APC.APC[name],sensoritem,name))
                else:
                    _LOGGER.error('Sensor not available:{}'.format(sensoritem))
            add_entities(entities, True)
        else:
             _LOGGER.error('resources not given in configuration')
    else:
        _LOGGER.error('Name not given in configuration')
...
class APCUPSdSensor(Entity):
    ...
    def __init__(self, data, sensor_type,Device):
        ...
        self._name = Device + SENSOR_TYPES[sensor_type][0]
